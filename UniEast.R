...
morpho.data<-morpho.data[morpho.data$Taxon == "unicolor" | morpho.data$Taxon == "griscomi", ]
...
#
postscript("eastgroups.eps", horizontal = FALSE, onefile= FALSE, paper = "special", width = 10, height = 10)
...
points(2, max.BIC-BIC.Best.Model.Per.G[2], pch=21, bg="black", cex=2)
...
arrows(2, 1000, 
       2,max.BIC-BIC.Best.Model.Per.G[2], length=0, lty=3, lwd=1.5)
dev.off()
#
################################################################################################################
...
morpho.groups.colors <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442")
...
#
#
#obtain best mclust model with 2 groups
Mcluster.morpho.data.ln.pca.subset.G2 <- Mclust(morpho.data.ln.pca$scores, G=2)
#PCplot with 2 groups
morpho.groups.colors <- c("#999999", "#E69F00")
col.BestModel <- rep(morpho.groups.colors[1], length(morpho.data$Taxon)) 
col.BestModel[Mcluster.morpho.data.ln.pca.subset.G2$classification==2] <- morpho.groups.colors[2]
#gather for all morphological groups in the best Mclust model estimates of
#i) the mean vector and ii) the variance-covariance matrix
m.G2.all <- Mcluster.morpho.data.ln.pca.subset.G2$parameters$mean #the mean vector
Z.G2.all <- Mcluster.morpho.data.ln.pca.subset.G2$parameters$variance$sigma #the variance-covariance matrices

trait.x <- 1
trait.y <- 2
plot(Mcluster.morpho.data.ln.pca.subset.G2$data[,trait.x], Mcluster.morpho.data.ln.pca.subset.G2$data[,trait.y],
     xlab=colnames(Mcluster.morpho.data.ln.pca.subset.G2$data)[trait.x], ylab=colnames(Mcluster.morpho.data.ln.pca.subset.G2$data)[trait.y],
     cex.axis=1.5, cex.lab=1.5, bty="n", col=col.BestModel, pch=21, xlim=c(-4.5,4), ylim=c(-4,3),
     cex=1.5, lwd=2)
for(i in 1:Mcluster.morpho.data.ln.pca.subset.G2$G)
{
  points(ellipse(Z.G2.all[c(trait.x,trait.y),c(trait.x,trait.y),i], centre = m.G2.all[c(trait.x,trait.y),i], level = 0.95, npoints = 10000), type="l")
}
...
plot(c(0,1), c(0,1), type="n", axes=F, bty="n", 
     xlab="", ylab="")
legend(0.1, 1,
       paste("Morphological group", 1:2),
       col=morpho.groups.colors[1:2], 	pch=21, pt.lwd=2, pt.cex=1.5, cex=1.45, bty="o")
#export 3" x 4" PDF
...
plot(c(0,1), c(0,1), type="n", axes=F, bty="n", 
     xlab="", ylab="")
legend(0.1, 1,
       c(expression(italic("unicolor")), expression(italic("griscomi"))),
...
postscript("eastbargraph.eps", horizontal = FALSE, onefile= FALSE, paper = "special", width = 10, height = 10)
par(mfrow=c(1,5))
par(oma=c(4,4,0,0))
par(mar=c(2,2,2,1))

hist(Mcluster.morpho.data.ln.pca.subset.G2$classification[morpho.data$Taxon=="unicolor"], 
     breaks=seq(0.75, Mcluster.morpho.data.ln.pca.subset.G2$G+0.25, 0.5),
     main=expression(italic("unicolor")), xlab="", ylab="",
     cex.lab=1.5, cex.axis=1.5, col=morpho.groups.colors.hist, xaxt="n") 
axis(1, at=1:Mcluster.morpho.data.ln.pca.subset.G2$G, labels=T)

hist(Mcluster.morpho.data.ln.pca.subset.G2$classification[morpho.data$Taxon=="griscomi"], 
     breaks=seq(0.75, Mcluster.morpho.data.ln.pca.subset.G2$G+0.25, 0.5),
     main=expression(italic("griscomi")), xlab="", ylab="",
     cex.lab=1.5, cex.axis=1.5, col=morpho.groups.colors.hist, xaxt="n") 
axis(1, at=1:Mcluster.morpho.data.ln.pca.subset.G2$G, labels=T)
